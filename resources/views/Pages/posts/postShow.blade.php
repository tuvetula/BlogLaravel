                ...
                <div class="row">
                    <div class="col-md-9">
                        @foreach($post->tags as $tag)
                            <span
                                class="border border-primary rounded-pill p-2 bg-primary text-light align-middle">{{ $tag->name }}</span>
                        @endforeach
                    </div>
        ...
        <section class="card text-center my-3">
            ...
            <div class="card-content container mb-3">
                @foreach($post->comments as $comment)
                    <div class="container my-3">
                        <div class="row">
                            <p class="text-left font-weight-bold">@if($comment->commentable_type == 'App\Models\Admin')
                                    <span class="font-italic">{{ __('Administrator') }}</span> @endif{{ $comment->commentable->first_name }}
                                , le {{ date('d-m-Y',strtotime($comment->updated_at)) }}
                                à {{ date( 'H:i',strtotime($comment->updated_at)) }}</p>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-md-3 px-0">
                                <div class="row float-right mr-1">
                                    @if($session_id == $comment->commentable->id && $comment->commentable_type == $session_model)
                                        <a class="btn btn-sm btn-warning ml-2"
                                           href="{{ route('comment.edit' , $comment->id) }}">{{ __('Edit') }}</a>
                                    @endif
                                    @if($session_id == $comment->commentable->id && $comment->commentable_type == $session_model || $session_model == 'App\Models\Admin')
                                        <form action="{{route('comment.destroy' , $comment->id)}}" method="POST"
                                              ...
                                              onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer votre commentaire?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <div class="row">
                            @foreach($comment->tags as $tag)
                                <span
                                    class="border border-primary rounded-pill p-2 bg-primary text-light align-middle mr-1">{{ $tag->name }}</span>
                            @endforeach
                        </div>
                    </div>
                @endforeach
                <form class="bg-light p-3" action="{{ route('comment.store', $post->id) }}" method="POST">
                    ...
                    <div class="form-group my-3">
                        <textarea placeholder="Ajouter un commentaire..."
                                  class="form-control @error('comment') is-invalid @enderror" name="comment" id=""
                                  cols="60" rows="5">{{ old('comment') }}</textarea>
                        @error('comment')
                    ...
                    <div class="form-group text-left">
                        ...
                        <div class="container py-3">
                            <div id="listTags" class="row px-3"></div>
                        </div>
                    </div>
